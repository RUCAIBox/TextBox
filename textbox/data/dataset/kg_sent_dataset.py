...
class KGSentenceDataset(AbstractDataset):
    ...
    def _load_kg_data(self, dataset_path):
        ...
        return text_data, entity_data, mention_data, triple_data, relation_data

    def _load_source_data(self):
        for i, prefix in enumerate(['train', 'valid', 'test']):
            filename = os.path.join(self.dataset_path, f'{prefix}.src')
            text_data, entity_data, mention_data, triple_data, relation_data = self._load_kg_data(filename)
    ...
    def _text2idx(self):
        ...
        self.source_triple_idx = [[[([self.source_entity_token2idx[x] for x in h], self.source_relation_token2idx[r], [self.source_entity_token2idx[x] for x in t]) for h, r, t in doc] for doc in group] for group in self.source_triple]
