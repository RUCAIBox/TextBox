    ...
    def __call__(self, samples: List[Dict[str, List[int]]]) -> Dict[str, torch.Tensor]:
        ...
        for sample in samples:
            ...
            src_id = (
                ...
                if self.set != "test" and self.is_casual_model else sample["source_ids"]
            )
        ...
        if self.mask_ratio:
            batch["source_ids"], batch["target_ids"] = self.add_whole_word_mask(batch["source_ids"], do_permutate)
