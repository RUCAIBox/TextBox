...
from transformers import BertPreTrainedModel, PreTrainedModel, load_tf_weights_in_bert
...
class PreTrainedBertModel(BertPreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        Params:
            ...
            *model_args, **kwargs: additional input for the specific Bert class
                (ex: num_labels for BertForSequenceClassification)
        """
        # get parameters
        state_dict = {} if "from_scratch" in kwargs else None
        ...
        if 'num_labels' not in kwargs:
            kwargs['num_labels'] = 2
        if 'num_rel' not in kwargs:
            kwargs['num_rel'] = 0
        if 'type_vocab_size' not in kwargs:
            if 'new_segment_ids' in kwargs and kwargs['new_segment_ids']:
                kwargs['type_vocab_size'] = 6 + (1 if 's2s_add_segment' in kwargs and kwargs['s2s_add_segment'] else 0)
            else:
                kwargs['type_vocab_size'] = 2
        if 'task_idx' not in kwargs:
            kwargs['task_idx'] = 3
        if 'num_sentlvl_labels' not in kwargs:
            kwargs['num_sentlvl_labels'] = 2 if 'has_sentence_oracle' in kwargs and kwargs['has_sentence_oracle'] else 0
        if 'relax_projection' not in kwargs:
            kwargs['relax_projection'] = 4 if 'relax_projection' in kwargs and kwargs['relax_projection'] else 0
        ...
        arg_save = ['num_labels', 'num_rel', 'num_sentlvl_labels']
        kwargs = {key: value for key, value in kwargs.items() if key in arg_save}
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    config = BertConfig(vocab_size=32000, hidden_size=768,
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
        super(BertForPreTrainingLossMask, self).__init__(config)
            # BertConfig(vocab_size=config.vocab_size, hidden_size=config.hidden_size,
            ...
            #            max_position_embeddings=config.max_position_embeddings, type_vocab_size=config.type_vocab_size,
            #            relax_projection=0, new_pos_ids=False, initializer_range=0.02, task_idx=None, ffn_type=0,
            #            label_smoothing=None))
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(
