...
Some modules of the table2text model are stored in this file
Contains Decoder, Encoder, Embeddings, BeamSearch, Attention,TextGeneration module 
"""
...
class Translator(object):
    ...
    def translate_batch(self, batch, data, stage1):
        ...
        beam = [Beam(beam_size, n_best=self.n_best, cuda=self.cuda, global_scorer=self.global_scorer,
                     pad=self.pad, eos=self.eos, bos=self.sos, min_length=self.min_length,
                     stepwise_penalty=self.stepwise_penalty) for __ in range(batch_size)]
...
class GlobalAttention(nn.Module):
    ...
    Args:
       type(string): user which attention mechanism
       dim (int): dimensionality of query and key
       coverage (bool): use coverage term
       attn_type (str): type of attention to use, options [dot,general,mlp]

    """
    def __init__(self, type, dim, coverage=False, attn_type="general"):
    ...
    def forward(self, input, memory_bank, memory_lengths=None, coverage=None):
        ...
        elif self.type == 'PointerAttention':
            align_vectors = self.sm(align.view(batch * targetL, sourceL))
            align_vectors = align_vectors + 1e-8
            align_vectors = torch.log(align_vectors)
            align_vectors = align_vectors.view(batch, targetL, sourceL)
            align_vectors = align_vectors.transpose(0, 1).contiguous()
...
class GlobalSelfAttention(nn.Module):
    ...
    def score(self, h_t, h_s):
        ...
        if self.attn_type in ["general", "dot"]:
            if self.attn_hidden >  0:
                h_t = self.transform_in(h_t)    # dim->attn_hidden
    ...
    def forward(self, input, memory_bank, memory_lengths=None, coverage=None):
        ...
        batch, sourceL, dim = memory_bank.size()
        batch_, targetL, dim_ = input.size()
        align = self.score(input, memory_bank)    # `[batch x tgt_len x src_len]`
        mask = sequence_mask(memory_lengths)
        mask = mask.unsqueeze(1)  # Make it broadcastable.
