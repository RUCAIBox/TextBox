...
class ScheduledOptim(AbstractOptim):
    def __init__(self, base_optimizer: torch_optim, init_lr: float, d_model: int, n_warmup_steps: int):
        super().__init__(base_optimizer, init_lr * d_model ** (-0.5))
        self.warmup_k = n_warmup_steps ** 1.5

    @property
    def lr(self):
        return self.init_lr * min(self.n_steps ** (-0.5), self.warmup_k * self.n_steps)
